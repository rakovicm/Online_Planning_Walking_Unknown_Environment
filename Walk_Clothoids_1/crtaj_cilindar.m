function []=crtaj_cilindar(predmet)

% funkcija za crtanje cilinda
...
% 'or'
    try
        R=predmet.dim(1);
        visina=predmet.dim(2);

        % if (strcmp(velicina,'veliki'))==true
        %     R=0.08;
        % else
        %     R=0.034;
        % end
        % 
        % 
        % if (strcmp(debljina,'debeli'))==true
        %     visina=0.015;
        % else
        %     visina=0.008;
        % end
        ...
        N=200;
        r1=[predmet.poz(1) predmet.poz(2) 0];
        r2=[predmet.poz(1) predmet.poz(2) visina];

        center = [r1(1),r1(2),r1(3)];
        radius = R;
        NOP = 1000;
        ...
        h = fill3(X,Y,Z,predmet.boja); alpha(h,0.8);

        center = [r2(1),r2(2),r2(3)];
        radius = R;
        NOP = 1000;
        THETA=linspace(0,2*pi,NOP);
        RHO=ones(1,NOP)*radius;
        ...
        patch('Vertices',v,'Faces',TRI,'facecolor',predmet.boja,'facealpha',0.8);
        % view(3);
        axis equal; 
        % grid on; 
        % title('Enclosed Cylinder')
