...
% % % % % % %    %%SIMULACIJA - NASTAVAK
%     load('sim1_faza6b1v1d1n.mat');    
%     tk=50;%ukupno vreme simulacije
%     brit=round(tk/dt);%Ukupan broj iteracija
%     t=(0:brit-1)*dt;%ukupno vreme simulacije
...
% % %         
    ...
    %     end
        ...
        if(check_point == 1)
            ...
            c_pose(3) = rbazor(3);
            goal = [8 0 0];
            [xp yp putanja sece] = search_point_for_clatoid(c_pose, goal,walls);
            if(sece == 1)
                k_usporti = 0.8;                
            else
                k_usporti = 1;
...
%            poz_ort_D(:,iii) = vec_distance_D(:,iii)/VecMod2(vec_distance_D(:,iii)); %#ok<AGROW>
        ...
        brzinakoraka = k_usporti*k_brz*brzinakoraka_nom;        
