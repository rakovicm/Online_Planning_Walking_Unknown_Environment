    ...
    B. Set the path by doing one of the following:
        ...
        * Add the MCR directory to the environment variable by issuing 
        ...
        NOTE: When deploying standalone applications, it is possible 
              to run the shell script file run_k_inemat.sh 
...
3. Collect the following files, based on what you are deploying: 
    ...
    NOTE: k_inemat.ctf will be present only if you have chosen 
    ...
    All Platforms:
        * k_inemat.ctf (if present)(Component Technology File archive,
          platform-dependent on target machine)

    Windows:
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          installer)
        * k_inemat.exe (application created by MATLAB Compiler)

    UNIX:
        * MCRInstaller.bin (MATLAB Compiler Runtime installer)
        * k_inemat (application created by MATLAB Compiler)
        * run_k_inemat.sh (shell script that can be run to 
    ...
    All Platforms:
        ...
        * k_inemat.dll (shared library, file extension varies by 
          platform)
        * k_inemat.h (library header file)
    ...
    Windows:
        * k_inemat.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)    
        * k_inemat.xml (documentation files)
        * k_inemat.pdb (program database file - if DEBUG option 
          selected)
        * k_inemat.dll (component assembly file)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
    ...
    Windows:
        * k_inemat.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)
        * k_inemat_1_0.dll (COM component)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)      
    ...
    All Platforms: 
        * k_inemat.jar (Java class files and compiled M-code)
    ...
    Windows:
        * k_inemat.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)
        * k_inemat_1_0.dll (COM component)
        * k_inemat.xla (optionally created and placed in
