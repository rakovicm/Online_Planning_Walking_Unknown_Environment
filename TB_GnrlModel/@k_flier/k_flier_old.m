...
for c_chain = 1:a.ch	% joints are in different chains
	...
	for c_si = (a.bs(c_chain)+1):a.cl(c_chain)		% cycle though the current chain
		...
		if( i>6 )
			if(a.su(c_chain,ip))
				tap = cross( -a.lne(:,ip,c_chain) , cross( a.alpr(:,a.su(c_chain,ip)) , a.lne(:,ip,c_chain) ) );
			else
				tap = cross( -a.lne(:,ip,c_chain) , cross( a.lpr(:,ip,c_chain) , a.lne(:,ip,c_chain) ) );
			end
			if(a.sl(i))
				tan = cross( a.le(:, i) , cross( a.aldr(:,a.sl(i)), a.le(:, i) ) );
			else
				tan = cross( a.le(:, i) , cross( a.ldr(:,i), a.le(:, i) ) );
			end
			tap = tap / vmod(tap);
			tan = tan / vmod(tan);
			tbp = cross(a.lne(:,ip,c_chain), tap);
			tbn = cross(a.le(:,i), tan);
