...
[Xm,Ym] = meshgrid(-5:.2:10);
Zm=0*Xm;
% mesh(Xm,Ym,Zm-0.005,0*Zm+0);
...
%     or=eye(3);
    ...
    line('XData',[rkt(1) xosa(1)],'YData',[rkt(2) xosa(2)],...
		'ZData',[rkt(3) xosa(3)],'Color','r','LineWidth',LineWKS); 

    line('XData',[rkt(1) yosa(1)],'YData',[rkt(2) yosa(2)],...
		'ZData',[rkt(3) yosa(3)],'Color','g','LineWidth',LineWKS); 

    line('XData',[rkt(1) zosa(1)],'YData',[rkt(2) zosa(2)],...
		'ZData',[rkt(3) zosa(3)],'Color','b','LineWidth',LineWKS); 
...
% 	surf(X+RV(1), Y+RV(2), Z+RV(3));	% sphere in the output joint
	line('XData',[TV(1) DV(1)],'YData',[TV(2) DV(2)],...
		'ZData',[TV(3) DV(3)],'Color',Col(1),'LineWidth',LineW);    
	line('XData',[TV(1) RV(1)],'YData',[TV(2) RV(2)],...
		'ZData',[TV(3) RV(3)],'Color',Col(3),'LineWidth',LineW);    
    if(i==3)
        RVskzr=RV;
    end
    if(i==8)
...
if(exist('df'))
	if(size(df)>1)
    	...
    	line('XData',[RVskzr(1) (rpeta_1(1)+rpeta_2(1))/2],'YData',[RVskzr(2) (rpeta_1(2)+rpeta_2(2))/2],...
        	'ZData',[RVskzr(3) (rpeta_1(3)+rpeta_2(3))/2],'Color',Col(1),'LineWidth',LineWF);        
		mesh(Xf+HV(1), Yf+HV(2), Zf+HV(3),'EdgeColor','k','LineWidth',LineWF);	% draw one foot
    	...
    	line('XData',[RVskzl(1) (lpeta_1(1)+lpeta_2(1))/2],'YData',[RVskzl(2) (lpeta_1(2)+lpeta_2(2))/2],...
        	'ZData',[RVskzl(3) (lpeta_1(3)+lpeta_2(3))/2],'Color',Col(1),'LineWidth',LineWF);              
	end
end
...
if(exist('dtoe'))
	if(size(dtoe)>1)
		...
		mesh(Xf+HV(1), Yf+HV(2), Zf+HV(3),'EdgeColor','k','LineWidth',LineWF);	% draw the other foot
	end
end
