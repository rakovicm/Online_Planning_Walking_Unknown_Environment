function [a, J, A] = k_Jsb(a,dq,varargin)
...
% Usage : [a, J, A] = k_Jsb(a, dq[, contact_index])
...
rc = Q * a.contact(:,ci);
J = [	[1 0 0]' [0 1 0]' [0 0 1]' cross(e(:,1),rc) cross(e(:,2),rc) cross(e(:,3),rc)
	zeros(3) e(:,1) e(:,2) e(:,3) ];

% Adjoint vector
A = zeros(6,1);
A(1:3,1) = a.dq(6)*cross( cross( a.omg(:,5),e(:,3) ),rc ) + ...
	a.dq(5)*cross( cross( a.omg(:,4),e(:,2) ),rc ) + ...
	cross( a.omg(:,6),cross( a.omg(:,6),rc ) );
A(4:6,1) = a.dq(5)*cross( a.omg(:,4),e(:,2) ) + a.dq(5)*cross( a.omg(:,4),e(:,2) );

% to the axes of the s-coordinate frame
tQ = a.cQ(:,:,ci);
sQ = Q * tQ;
